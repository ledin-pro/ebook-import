---
name: ebook-import
description: Import EPUB, standalone or compressed FB2, and DRM-free MOBI/AZW/AZW3 books into an Obsidian-compatible, AI-agent-readable Markdown corpus with an explicit per-book image policy. Always use this skill when a user asks to import, archive, mirror, convert, or make ebook files searchable for an agent, including .epub, .fb2, .fb2.zip, .fbz, .mobi, .azw, and .azw3. Before importing each book, ask whether to import or skip images. Preserve source order and never summarize or interpret source text during import. Route explicit Docling or OCR/image-text ebook requests to the separate docling-ebook skill. Do not use for PDF, scanned documents, KFX, AZW4, PRC, or DRM removal.
...
1. Confirm each input is EPUB, FB2, FB2.ZIP/FBZ, MOBI, AZW, or AZW3. Route PDF and scans to document workflows. Reject KFX, AZW4, PRC, and DRM-removal requests.
   When the user explicitly requests Docling output, Docling chunking, OCR, or image-text extraction from an ebook, use the separate `docling-ebook` skill and CLI instead. Do not reproduce that behavior in this workflow.
...
8. Report generated directories, formats, chapter counts, image modes, source hashes, conversion provenance, warnings, manifest paths, and validation observations.
9. Do not summarize, classify, translate, or interpret book text. Those are separate downstream tasks.
...
media, warnings, and MOBI conversion provenance.
...
chapter/media links, image mode, MOBI backend provenance, warnings, and UTF-8
output. Treat converter or parser warnings as reportable fidelity information,
not book content to reinterpret.
